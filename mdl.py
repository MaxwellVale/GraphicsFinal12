...
def p_command_path(p):
    ...
    cmd = {'op' : p[1], 'args' : p[3:13], 'pathname': p[2]}
    symbols[p[2]] = ['pathname', 0]
    commands.append(cmd)
...
def p_command_move(p):
    """command : MOVE NUMBER NUMBER NUMBER SYMBOL
               ...
               | MOVE SYMBOL"""
    if len(p) == 3:
        cmd = {'op': p[1], 'path': p[2], 'args' : []}
        commands.append(cmd)
    else:
        cmd = {'op' : p[1], 'args' : p[2:5], 'knob' : None, 'path': None}
        if len(p) == 6:
            cmd['knob'] = p[5]
            symbols[p[5]] = ['knob', 0]
        commands.append(cmd)
...
def p_command_vary(p):
    ...
    cmd = {'op' : p[1], 'args' : p[3:7], 'knob' : p[2], 'type' : 'LINEAR'}
    if len(p) == 8:
        cmd['type'] = p[7]
    symbols[p[2]] = ['knob', 0]
    commands.append(cmd)
